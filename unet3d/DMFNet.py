...
import torch
from sync_batchnorm.comm import *
...
class MFNet(nn.Module): #
    ...
    def __init__(self, c=4,n=32,channels=128,groups = 16,norm='bn', num_classes=1, activation='sigmoid', auxiliary=False):
    ...
    def forward(self, x):
        ...
        if not self.auxiliary:
            ...
            auxiliary = self.auxiliary_cls(x4) 
            return y4, auxiliary

class DMFNet(MFNet): # softmax
    # [128]  Flops:  27.045G  &  Params: 3.88M
    def __init__(self, c=4,n=32,channels=128, groups=16,norm='bn', num_classes=1, activation='sigmoid', auxiliary=False):
        super(DMFNet, self).__init__(c,n,channels,groups, norm, num_classes, activation, auxiliary)
...
if __name__ == '__main__':
    ...
    model = DMFNet(c=1, groups=16, norm='sync_bn', num_classes=7, activation='softmax', auxiliary=False)
    model.cuda(device)
